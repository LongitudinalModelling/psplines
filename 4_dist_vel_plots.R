...
graphics.off()
grDevices::cairo_pdf("res/fig_psme_dist_curves.pdf", height = 5, width = 4.5)
...
for (p in plot_params) {
  mplot(x = age^2, y = p$data[[p$y_col]], id = id, data = p$data, col = id, las = 1,
        ylim = p$ylim, xlab = 'age - years', ylab = p$y_label, axes = F)
  lines(p$age_data^2, xTraj(p$pop_data, p$age_data)$pop, type = "l", col = 'black', lwd = 2)
  axis(1)
  axis(2, las = 1)
  box(bty = "l")
