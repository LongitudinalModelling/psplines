...
source("src/7_1_dp_helper_functions.R")
...
dp_pred_MF <- lapply(sex, function(params) {
  ...
  df_list <- map(measurements, ~{
    ...
    as.data.frame(get(object_name)$fitted.ind.grid) %>%
      transmute(id = case, age = xgrid, !!new_col_name := fitted)
  })
...
dp_est_M <- get_pBMI(dp_pred_M, sqrt(1.5)) %>%
  mutate(id = unique(dp_pred_M$id), sex = "Boys", method = "double penalty") %>%
  rename(PeakBMIAge = x, PeakBMI = y) %>% mutate(PeakBMIAge = PeakBMIAge^2)

dp_est_M <- get_rBMI(dp_pred_M, sqrt(2)) %>%
  mutate(id = unique(dp_pred_M$id)) %>% rename(ReboundBMIAge = x, ReboundBMI = y) %>% 
  mutate(ReboundBMIAge = ReboundBMIAge^2) %>%  full_join(dp_est_M)

dp_est_M <- get_phv(dp_pred_M, sqrt(1)) %>%
  mutate(id = unique(dp_pred_M$id)) %>% 
  rename(APHV = x, PHV = y) %>% mutate(
    PHV = PHV / (2 * median(APHV, na.rm = T)),
    APHV = (APHV^2 * 52.1429)) %>% full_join(dp_est_M)

dp_est_M <- get_pwv(dp_pred_M, sqrt(1)) %>%
  mutate(id = unique(dp_pred_M$id)) %>% 
  rename(APWV = x, PWV = y) %>% mutate(
    PWV = PWV / (2 * median(APWV, na.rm = T)),
    APWV = (APWV^2 * 52.1429)) %>% full_join(dp_est_M)

# PSME

psme_est_M <- get_pBMI(psme_pred_M, sqrt(1.5)) %>%
  mutate(id = unique(psme_pred_M$id), sex = "Boys", method = "psme penalty") %>%
  rename(PeakBMIAge = x, PeakBMI = y) %>% mutate(PeakBMIAge = PeakBMIAge^2)

psme_est_M <- get_rBMI(psme_pred_M, sqrt(2)) %>%
  mutate(id = unique(psme_pred_M$id)) %>% rename(ReboundBMIAge = x, ReboundBMI = y) %>% 
  mutate(ReboundBMIAge = ReboundBMIAge^2) %>%  full_join(psme_est_M)

psme_est_M <- get_phv(psme_pred_M, sqrt(1)) %>%
  mutate(id = unique(psme_pred_M$id)) %>% 
  rename(APHV = x, PHV = y) %>% mutate(
    PHV = PHV / (2 * median(APHV, na.rm = T)),
    APHV = (APHV^2 * 52.1429)) %>% full_join(psme_est_M)

psme_est_M <- get_pwv(psme_pred_M, sqrt(1)) %>%
  mutate(id = unique(psme_pred_M$id)) %>% 
  rename(APWV = x, PWV = y) %>% mutate(
    PWV = PWV / (2 * median(APWV, na.rm = T)),
    APWV = (APWV^2 * 52.1429)) %>% full_join(psme_est_M)

# JOIN

psme_dp_est_M <- full_join(dp_est_M, psme_est_M)

rm(get_peak, get_trough, dp_est_M, psme_est_M, dp_pred_M, psme_pred_M, dp_pred_F, 
   psme_pred_F, get_pBMI, get_rBMI, get_vel, get_phv, get_pwv)
...
print(psme_dp_est_M %>% mutate(PHV = PHV * 0.0833334, PWV = PWV *  83.3334,
               PeakBMIAge = PeakBMIAge * 12) %>% group_by(method) %>% summarise(
...
psme_dp_est_M <- psme_dp_est_M %>% mutate(
...
with(psme_dp_est_M, plot(`PHV_psme penalty`, `PHV_double penalty`, xlab = "psme penalty", ylab = "double penalty", xlim=c(3, 5.9), ylim=c(3, 5.9)))
title(paste("a. Peak height velocity (cm/mo), r =", round(with(psme_dp_est_M, cor(`PHV_psme penalty`, `PHV_double penalty`, use='complete.obs')), 2)), adj = 0, line = 0.6)
abline(a = 0, b = 1, col = "red")

with(psme_dp_est_M, plot(`PWV_psme penalty`, `PWV_double penalty`, xlab = "psme penalty", ylab = "double penalty", xlim=c(595, 1850), ylim=c(595, 1850)))
title(paste("b. Peak weight velocity (g/mo), r =", round(with(psme_dp_est_M, cor(`PWV_psme penalty`, `PWV_double penalty`, use='complete.obs')), 2)), adj = 0, line = 0.6)
abline(a = 0, b = 1, col = "red")
...
with(psme_dp_est_M, plot(`PeakBMIAge_psme penalty`, `PeakBMIAge_double penalty`, xlab = "psme penalty", ylab = "double penalty", xlim=c(2, 18), ylim=c(2, 18)))
title(paste("e. Age at peak BMI (mo), r =", round(with(psme_dp_est_M, cor(`PeakBMIAge_psme penalty`, `PeakBMIAge_double penalty`, use='complete.obs')), 2)), adj = 0, line = 0.6)
abline(a = 0, b = 1, col = "red")

with(psme_dp_est_M, plot(`ReboundBMIAge_psme penalty`, `ReboundBMIAge_double penalty`, xlab = "psme penalty", ylab = "double penalty", xlim=c(1.5, 10), ylim=c(1.5, 10)))
title(paste("f. Age at rebound BMI (y), r =", round(with(psme_dp_est_M, cor(`ReboundBMIAge_psme penalty`, `ReboundBMIAge_double penalty`, use='complete.obs')), 2)), adj = 0, line = 0.6)
abline(a = 0, b = 1, col = "red")
