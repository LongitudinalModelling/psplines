...
# MISSING: m_age 0, m_ht 18, m_wt 51, GA 0, BWT 0, BLT 1

psme_est_MF <- psme_est_MF %>% filter(
  !is.na(m_ht) & !is.na(m_wt) & !is.na(BLT)) # 686
...
out <- c("PHV", "PWV", "PeakBMI", "ReboundBMI", "PeakBMIAge", "ReboundBMIAge * 12") 
...
(p1 <- assoc_plot(psme_assoc_res, "a. Peak height velocity", "cm/month", "a. Peak height velocity"))
(p2 <- assoc_plot(psme_assoc_res, "b. Peak weight velocity", "grams/month", "b. Peak weight velocity"))
