...
#------------------------------------------------------#
#### GET GROWTH FEATURES: PEAK/REBOUND BMI, PHV/PWV ####
#------------------------------------------------------#
...
est_M <- psme_pred_M %>% select(id, age, cht) %>% 
  filter(age <= sqrt(1)) %>% mutate(age = age^2) %>% 
  get_pv("id", "age", "cht") %>% select(-APV) %>% 
  rename(PHV = PV) %>% full_join(est_M)

est_M <- psme_pred_M %>% select(id, age, cwt) %>% 
  filter(age <= sqrt(1)) %>% mutate(age = age^2) %>% 
  get_pv("id", "age", "cwt") %>% select(-APV) %>% 
  rename(PWV = PV) %>% full_join(est_M)
...
est_F <- psme_pred_F %>% select(id, age, cht) %>% 
  filter(age <= sqrt(1)) %>% mutate(age = age^2) %>% 
  get_pv("id", "age", "cht") %>% select(-APV) %>% 
  rename(PHV = PV) %>% full_join(est_F)

est_F <- psme_pred_F %>% select(id, age, cwt) %>% 
  filter(age <= sqrt(1)) %>% mutate(age = age^2) %>% 
  get_pv("id", "age", "cwt") %>% select(-APV) %>% 
  rename(PWV = PV) %>% full_join(est_F)

rm(get_pBMI, get_rBMI, get_pv, psme_pred_M, psme_pred_F)
