...
source("src/6_1_dp_helper_functions.R")
...
dp_est_M <- dp_pred_M %>% select(id, age, cht) %>% 
  filter(age <= sqrt(1)) %>% mutate(age = age^2) %>% 
  get_pv("id", "age", "cht") %>% select(-APV) %>% 
  rename(PHV = PV) %>% full_join(dp_est_M)

dp_est_M <- dp_pred_M %>% select(id, age, cwt) %>% 
  filter(age <= sqrt(1)) %>% mutate(age = age^2) %>% 
  get_pv("id", "age", "cwt") %>% select(-APV) %>% 
  rename(PWV = PV) %>% full_join(dp_est_M)
...
psme_est_M <- psme_pred_M %>% select(id, age, cht) %>% 
  filter(age <= sqrt(1)) %>% mutate(age = age^2) %>% 
  get_pv("id", "age", "cht") %>% select(-APV) %>% 
  rename(PHV = PV) %>% full_join(psme_est_M)

psme_est_M <- psme_pred_M %>% select(id, age, cwt) %>% 
  filter(age <= sqrt(1)) %>% mutate(age = age^2) %>% 
  get_pv("id", "age", "cwt") %>% select(-APV) %>% 
  rename(PWV = PV) %>% full_join(psme_est_M)
...
rm(get_peak, get_trough, dp_est_M, psme_est_M, dp_pred_M, psme_pred_M, dp_pred_F, 
   psme_pred_F, get_pBMI, get_rBMI, get_pv)
...
with(psme_dp_est_M, plot(`PHV_psme penalty`, `PHV_double penalty`, xlab = "psme penalty", ylab = "double penalty", xlim=c(80, 134), ylim=c(80, 134)))
...
with(psme_dp_est_M, plot(`PWV_psme penalty`, `PWV_double penalty`, xlab = "psme penalty", ylab = "double penalty", xlim=c(4, 48), ylim=c(4, 48)))
